...
/**
 * Meta storage layer for the ITT page content.
 */
final class ITT_Meta {

	/**
	 * Prefix for every meta key written by the theme.
	 */
	private const PREFIX = '_itt_';
	...
	public static function register(): void {
		foreach ( array_keys( ITT_Fields::all() ) as $section ) {
			register_post_meta(
				'page',
				self::key( $section ),
				array(
	...
	public static function get( string $section, ?int $post_id = null ): array {
		...
		$defaults = ITT_Content::section( $section );
	...
	public static function sanitize( string $section, array $raw ): array {
		return self::sanitize_fields( ITT_Fields::fields( $section ), $raw );
	}
	...
	private static function sanitize_fields( array $fields, array $raw ): array {
		...
		foreach ( $fields as $field ) {
			...
			$clean[ $key ] = self::sanitize_value( $field, $value );
		}
	...
	private static function sanitize_value( array $field, mixed $value ): mixed {
		return match ( $field['type'] ) {
			...
			'textarea', 'rich', 'lines' => sanitize_textarea_field( (string) ( is_scalar( $value ) ? $value : '' ) ),
			...
			'image', 'video' => absint( is_scalar( $value ) ? $value : 0 ),
