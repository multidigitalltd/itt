		...
		return array(
			...
			'account'  => self::section(
				...
				array(
					...
					self::checkbox( 'show_people', 'הצגת רשימת המשתתפים בעמוד הקבוצה', 'כבוי = עמוד הקבוצה מציג את המספר, את פס ההתקדמות ואת היצירה, בלי רשימת השמות. ההצטרפויות עצמן נשמרות כרגיל וממשיכות להיספר; זו החלטת תצוגה בלבד, וההדלקה מחזירה את הרשימה כפי שהייתה.' ),
					self::bi( 'text', 'eyebrow', 'שורת פתיחה קטנה' ),
			...
			'groups'   => self::section(
				...
				array(
					...
					self::bi( 'text', 'f_artwork_photo', 'בטופס — האפשרות לבנות את היצירה מהתמונה' ),
					self::bi( 'text', 'f_photo', 'בטופס — תווית העלאת התמונה' ),
					self::bi( 'text', 'f_photo_help', 'בטופס — ההסבר מתחת להעלאת התמונה', 3 ),
