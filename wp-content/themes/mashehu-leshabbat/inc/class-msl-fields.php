		...
		return array(
			...
			'campaign' => self::section(
				...
				array(
					...
					self::checkbox( 'moderate_dedications', 'הקדשות ממתינות לאישור לפני פרסום', 'כבוי (ברירת המחדל) = הקדשה מתפרסמת מיד עם ההצטרפות. הדלקה מחזירה תור: כל הקדשה ממתינה במסך "הקדשות" ואינה נספרת עד שתאושר. מסך האישור, הכפתורים והמספרים נשארים בכל מקרה.' ),
					self::checkbox( 'limit_joins', 'להגביל הצטרפות חוזרת מאותו אדם', 'כבוי (ברירת המחדל) = אפשר להדליק נר שוב ושוב, מאותו בית, מאותו משרד ומאותו טלפון, בלי הגבלה. הדלקה = חוזרים למגבלה: מייל או טלפון שכבר נרשמו נחסמים, וגם כתובת IP אחת עם אותו שם ועיר. בית, בית ספר או רשת סלולרית שלמה יושבים מאחורי כתובת אחת — ולכן ההגבלה סופרת קהל שלם כאדם אחד. מלכודת הבוטים וזמן המילוי המזערי פועלים בכל מקרה.' ),
					...
					self::number( 'dedications_seed', 'מספר הקדשות לפתיחה', 0, 1000000, 'המספר שממנו מתחילה משבצת "הקדשות". הקדשות אמיתיות שאושרו נוספות עליו. זה מספר תצוגה: הוא אינו טקסט של אף אחד, אינו מופיע בקיר ההקדשות ובייצוא, ואפס = רק מה שנכתב באמת.' ),
			...
			'join'     => self::section(
				...
				array(
					...
					self::bi( 'text', 'weeks_title', 'לכמה שבתות — השאלה' ),
					self::bi( 'text', 'weeks_1', 'לכמה שבתות — שבת אחת' ),
					self::bi( 'text', 'weeks_2', 'לכמה שבתות — שתיים' ),
					self::bi( 'text', 'weeks_4', 'לכמה שבתות — ארבע' ),
					self::bi( 'text', 'weeks_8', 'לכמה שבתות — שמונה' ),
					self::bi( 'textarea', 'weeks_help', 'לכמה שבתות — הסבר', 2 ),
					self::bi( 'text', 'mail_weekly_subject', 'מייל התזכורת — נושא' ),
					self::bi( 'textarea', 'mail_weekly_body', 'מייל התזכורת — גוף. %1$s הוא השם, %2$s הוא קישור לאתר', 8 ),
					self::bi( 'text', 'mail_again_subject', 'מייל הקבלה הנוספת — נושא' ),
					self::bi( 'textarea', 'mail_again_body', 'מייל הקבלה הנוספת — גוף. %1$s הוא השם, %2$s הוא קישור לאתר', 8 ),
					self::bi( 'text', 'mail_friend', 'פנייה במייל למי שלא השאיר שם' ),
			...
			'groups'   => self::section(
				...
				array(
					...
					self::checkbox( 'show_archive', 'הצגת רשימת הקבוצות באתר', 'כבוי = אין באתר עמוד שמרכז את כל הקבוצות: הרשימה אינה מוצגת, והקישורים אליה נעלמים מהתפריט, מההזמנה שבעמוד הקמפיין ומעמוד הקבוצה. קבוצה עדיין נפתחת ועדיין יש לה עמוד וכתובת משלה — מי שמחזיק את הקישור מגיע אליה כרגיל.' ),
					self::checkbox( 'show_people', 'הצגת רשימת המשתתפים בעמוד הקבוצה', 'כבוי = עמוד הקבוצה מציג את המספר, את פס ההתקדמות ואת היצירה, בלי רשימת השמות ובלי מתי כל אחד הדליק. ההצטרפויות עצמן נשמרות כרגיל וממשיכות להיספר; זו החלטת תצוגה בלבד.' ),
					self::checkbox( 'auto_approve', 'קבוצה חדשה עולה לאוויר מיד', 'דלוק (ברירת המחדל) = קבוצה שנפתחת מתפרסמת מיד, בלי לחכות לאף אחד. כיבוי מחזיר אישור: כל קבוצה ממתינה במסך "קבוצות", ועד שתאושר היא אינה מופיעה ברשימה — אך מי שפתח אותה ומי שקיבל ממנו את הקישור כן רואים אותה ויכולים להצטרף. הטקסט בקבוצה נכתב בידי גולשים ומוצג תחת השם של המיזם.' ),
					self::bi( 'text', 'eyebrow', 'שורת פתיחה קטנה' ),
					self::bi( 'text', 'title', 'כותרת העמוד' ),
					self::bi( 'textarea', 'lead', 'פסקת פתיחה', 3 ),
					...
					self::bi( 'text', 'single_open_art', 'בעמוד הקבוצה — כפתור פתיחת היצירה' ),
					self::bi( 'text', 'f_photo', 'בטופס — תווית העלאת תמונת הקאבר' ),
					self::bi( 'text', 'f_photo_help', 'בטופס — ההסבר מתחת לתמונת הקאבר', 3 ),
					self::bi( 'text', 'photo_alt', 'תיאור התמונה לקוראי מסך' ),
					self::bi( 'text', 'err_photo', 'שגיאה — התמונה לא הגיעה' ),
					self::bi( 'text', 'err_photo_big', 'שגיאה — התמונה גדולה מדי' ),
					self::bi( 'text', 'err_photo_type', 'שגיאה — הקובץ אינו תמונה' ),
					self::bi( 'text', 'err_photo_server', 'שגיאה — השרת לא הצליח לעבד את התמונה' ),
					self::bi( 'text', 'single_when', 'בעמוד הקבוצה — כותרת עמודת הזמן ברשימה' ),
					self::bi( 'text', 'ago_now', 'זמן — עכשיו' ),
					self::bi( 'text', 'ago_minute', 'זמן — לפני דקה' ),
					self::bi( 'text', 'ago_two_minutes', 'זמן — לפני שתי דקות' ),
					self::bi( 'text', 'ago_minutes', 'זמן — לפני כמה דקות', 3, '%d = מספר הדקות.' ),
					self::bi( 'text', 'ago_hour', 'זמן — לפני שעה' ),
					self::bi( 'text', 'ago_two_hours', 'זמן — לפני שעתיים' ),
					self::bi( 'text', 'ago_hours', 'זמן — לפני כמה שעות', 3, '%d = מספר השעות.' ),
					self::bi( 'text', 'ago_yesterday', 'זמן — אתמול' ),
					self::bi( 'text', 'ago_two_days', 'זמן — לפני יומיים' ),
					self::bi( 'text', 'ago_days', 'זמן — לפני כמה ימים', 3, '%d = מספר הימים.' ),
					self::bi( 'text', 'single_also', 'בעמוד הקבוצה — המשפט על היצירה הכללית' ),
					self::bi( 'text', 'single_back', 'בעמוד הקבוצה — חזרה לרשימה' ),
					self::bi( 'text', 'back_home', 'כפתור חזרה לעמוד הקמפיין', 3, 'מוצג במקום "לכל הקבוצות" כשרשימת הקבוצות כבויה.' ),
